...
function RSS_Display( $urls=array(), $size = 15, $site = 0, $image='' )
{
	...
	foreach($recents as $article)
	{
		...
		$page .= "
				<tr>
					<td class=\"text-bold\"><a href=\"$link\">" . (( strlen($title) > 30)? substr($title, 0, 30) . "...":$title) . "</a></td>
					<td class=\"text-bold text-right\">" . date('H:i A') . "</td>
				</tr>
				<tr>
					<td colspan=\"2\" class=\"padding\">" . (( strlen($description) > 70)? substr($description, 0, 70) . "...":$description) . "</td>
				</tr>";
	}
	$page .= "</table>";
